...
ggplot(cars_pln, aes(x = Displacement_cm3, y = Year_Group, fill = Year_Group)) +
  geom_density_ridges(alpha = 0.5, scale = 1.2) +
  scale_fill_viridis_d() +
  labs(title = "Ridgeline Plot of Displacement (cm3) Across Different Year Groups",
       x = "Displacement (cm3)",
       y = "Car Year Group",
       fill = "Car Year Group") +
  theme_minimal() +
...
# Wykres
ggplot(transmission_percentages, aes(x = Production_year, y = percentage, fill = Transmission)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_fill_manual(values = c("Manualna" = "#1f77b4", "Automatyczna" = "#ff7f0e")) +  
  theme_minimal() +
  labs(title = "Udział skrzyń manualnych i automatycznych według roku produkcji",
       x = "Rok produkcji",
       y = "Procent",
